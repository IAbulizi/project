...
```{r}
# plot 1-y expectation
inflation %>% 
  group_by(date) %>% 
  summarise(across(c(perception, expectation_1y),
                   ~ mean(., na.rm = TRUE)),
            .groups = "drop") %>% 
  pivot_longer(c(expectation_1y, perception)) %>% 
  ungroup() %>% 
  ggplot() +
...
# plot 2-y expectation
inflation %>% 
  group_by(date) %>% 
  summarise(across(c(perception, expectation_2y),
                   ~ mean(., na.rm = TRUE)),
            .groups = "drop") %>% 
  pivot_longer(c(expectation_2y, perception)) %>% 
  ungroup() %>% 
  ggplot() +
  geom_line(aes(date, value, color = name)) +
  theme_minimal() +
